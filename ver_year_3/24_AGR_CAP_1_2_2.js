...
function toggleCap2(trimValue) {
    if (trimValue == 4) {
        ...
        // Hide dynamic Cap 1.2 by targeting all dynamic rows that belong to CAP12_***_FILIAL
        ...
        // Clear only CAP12_***_FILIAL inputs in both static and dynamic Cap 1.2
        jQuery('input[name^="CAP12"]').val('');  // Clear inputs specifically for CAP12_***_FILIAL
    } else {
        // Show static Cap 1.2
        ...
        // Show dynamic Cap 1.2 by targeting all dynamic rows that belong to CAP12_***_FILIAL
        jQuery('input[name^="CAP12"]').each(function () {
            var rowIndex = jQuery(this).attr('row-index');  // Get the dynamic row index

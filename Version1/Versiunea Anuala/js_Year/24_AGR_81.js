    ...
    Drupal.behaviors.agro24 = {
        attach: function (context, settings) {
            ...
            jQuery('input[type=checkbox]').change(function () {
                ...
                if (group == "TRIM") {
                    var trimValue = jQuery(this).val(); // Assuming TRIM value is stored in the checkbox value

                    if (trimValue == 4) {
                        // Hide Chapter 1.2
                        jQuery('#header-1-2').hide();  // Hides the header for Chapter 1.2
                        jQuery('#CAP12').hide();       // Hides the entire table with id CAP12

                        // Clear all values in the table CAP12
                        jQuery('input[name^="CAP12"]').val('');            // Clears all text inputs related to CAP12
                        jQuery('input[name^="CAP12"]').attr('checked', false); // Unchecks all checkboxes related to CAP12


                    } else {
                        // Show Chapter 1.2 if TRIM is not 4
                        jQuery('#header-1-2').show();  // Shows the header for Chapter 1.2
                        jQuery('#CAP12').show();       // Shows the entire table with id CAP12

        ...
        for (var i = 0; i <= 6; i++) {
            if (i !== 2) {
                ...
                if (values["CAP111_R10_C" + i + "_FILIAL"] && !isNaN(Number(values["CAP111_R10_C" + i + "_FILIAL"][j]))) {
                    R9_C = Number(values["CAP111_R10_C" + i + "_FILIAL"][j]);
                }

                if (values["CAP12_R10_C" + i + "_FILIAL"] && !isNaN(Number(values["CAP12_R10_C" + i + "_FILIAL"][j]))) {
                    R8_C = Number(values["CAP12_R10_C" + i + "_FILIAL"][j]);
                }

                if ((R9_C !== 0 && R8_C === 0)  ) {
                    webform.errors.push({
                        'fieldName': R9_C !== 0 ? 'CAP12_R10_C' + i + '_FILIAL' : 'CAP111_R10_C' + i + '_FILIAL',
                        'index': j,
                        'weight': 19,
                        'msg': Drupal.t('Raion: @CAP_CUATM_FILIAL - Cod eroare: 33-008-F. [@col_FILIAL] - COL(@col_FILIAL), Dacă Rînd.(10) COL(1) ≠ 0 atunci Rînd.(10) COL(1) ≠ 0, @R9_C <> @R8_C', { '@CAP_CUATM_FILIAL': CAP_CUATM_FILIAL, '@col_FILIAL': i, '@R9_C': R9_C, '@R8_C': R8_C })
                    });
                }
            }
        }
